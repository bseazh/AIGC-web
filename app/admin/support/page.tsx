...
const tabs: Array<{ key: Tab; label: string; icon: typeof ShieldCheck }> = [
  { key: "complaints", label: "投诉处理", icon: TicketCheck },
...
export default function AdminSupportPage() {
  ...
  return <main className="admin-shell"><header><div><span><ShieldCheck size={17} />受限后台</span><h1>客服与用户处置</h1><p>处理投诉、用户状态、任务重试、授权记录和管理员审计。</p></div><div className="admin-header-actions"><button onClick={() => load()}><RefreshCw size={15} />刷新</button><Link className="admin-back-link" href="/workspace">返回工作台</Link></div></header>
