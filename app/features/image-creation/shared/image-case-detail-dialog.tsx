...
function preserveFullImage(url: string) {
  ...
  try {
    ...
    return url.replace(/fit=crop/g, "fit=max").replace(/w=\d+/, "w=1600").replace(/&h=\d+/, "");
  }
}
  ...
  return <div className="image-case-dialog-backdrop" role="dialog" aria-modal="true" aria-label={`${item.title}作品详情`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="image-case-dialog">
      <div className="image-case-dialog-media">
        <img src={preserveFullImage(images[index])} alt={`${item.title} 第 ${index + 1} 张`} />
      ...
      <aside className="image-case-dialog-detail">
        ...
        <section><h3>案例素材</h3><div className="image-case-reference-list">{references.map((reference, referenceIndex) => <figure key={`${reference.image}-${referenceIndex}`}><figcaption>{reference.label}</figcaption><img src={preserveFullImage(reference.image)} alt={reference.label} /></figure>)}</div></section>
