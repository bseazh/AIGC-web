...
import { ArrowLeft, Check, Download, Film, FolderOpen, LoaderCircle, Music2, Plus, Sparkles, Upload, X } from "lucide-react";
...
export function VideoWorkflowPage({ template = "seedance" }: Props) {
  ...
  return <main className="creator-shell video-creator-shell"><header className="creator-header"><button className="icon-button" aria-label="返回视频中心" onClick={() => router.push("/create/product-video")}><ArrowLeft size={19} /></button><div><strong>{config.title}</strong><span>{config.subtitle}</span></div><div className="creator-points"><Sparkles size={15} />{account.user.isAdministrator ? "管理员免积分" : `${account.wallet.availablePoints} 积分`}</div></header>
