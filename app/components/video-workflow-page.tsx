...
const allDefinitions: Definition[] = [
  ...
  },
  {
    key: "audio",
    label: "参考音频",
    hint: "可选 · MP3 / WAV，最多 30MB",
    accepts: "audio/mpeg,audio/mp3,audio/wav",
    icon: Music2,
  },
];
...
const templateConfig = {
  ad: {
    title: "产品广告大片",
    subtitle: "产品图片驱动的商业广告短片",
    submitUrl: "/api/tasks/product-ad-video/",
    description: "上传产品图，由模型自动识别材质、细节与卖点，生成广告镜头。",
    keys: imageKeys,
    scenes: ["产品广告大片"],
    styles: ["商业广告"],
  },
  recreate: {
    title: "复刻带货视频",
    subtitle: "参考节奏，生成原创商品内容",
    submitUrl: "/api/tasks/recreate-video/",
    description: "仅提取运镜、景别和转场结构，不复制原内容。",
    keys: ["image1", "video", "audio"] as UploadSlot[],
    scenes: ["镜头节奏复刻", "商品展示复刻", "种草讲解复刻", "场景切换复刻"],
    styles: ["自然带货", "轻快节奏", "质感种草", "促销转化"],
  },
  seedance: {
    title: "Seedance2 视频",
    subtitle: "图、视频、音频自由组合的高级创作",
    submitUrl: "/api/tasks/video/",
    description: "完整参考素材与自由脚本控制。",
    keys: ["image1", "image2", "video", "audio"] as UploadSlot[],
    scenes: ["商品特写", "第一人称", "生活方式", "自由创作"],
    styles: ["轻快节奏", "质感广告", "真实记录", "电影感"],
  },
  mix: {
    title: "智能混剪",
    subtitle: "多段授权视频保留原音频合成为成片",
    submitUrl: "/api/tasks/video-mix/",
    description:
      "至少选择两段已授权 MP4 视频。系统将裁切、拼接并保留原始音轨。",
    keys: ["video", "video2"] as UploadSlot[],
    scenes: ["原音频混剪"],
    styles: ["自然转场"],
  },
} as const;
...
export function VideoWorkflowPage({ template = "seedance" }: Props) {
  ...
  const definitions = allDefinitions
    ...
    .map((definition) => ({
      ...definition,
      required:
        definition.key === "image1" ||
        (template === "recreate" && definition.key === "video") ||
        (template === "mix" && ["video", "video2"].includes(definition.key)),
    }));
  ...
  const choose = (key: UploadSlot, file?: File) => {
    ...
    setUploads((current) => {
      if (current[key]?.file) URL.revokeObjectURL(current[key]!.preview);
      return {
        ...current,
        [key]: {
          file,
  ...
  const inputCard = (definition: Definition) => {
    const item = uploads[definition.key];
    const Icon = definition.icon;
    return (
      <label
        className={`video-upload-card ${item ? "has-file" : ""}`}
        key={definition.key}
      >
        {item && definition.key.startsWith("image") ? (
          <img src={item.preview} alt={definition.label} />
        ) : item ? (
          <div className="file-preview">
            <Icon size={28} />
            <strong>{item.name}</strong>
            <small>{Math.ceil(item.byteSize / 1024 / 1024)} MB</small>
          </div>
        ) : (
          <>
            <span>
        ...
        <input
          type="file"
          accept={definition.accepts}
          onChange={(event) => choose(definition.key, event.target.files?.[0])}
        />
      </label>
    );
  };
  ...
  return (
    <main className="creator-shell video-creator-shell">
      <header className="creator-header">
        ...
        <div className="creator-points">
          ...
          {account.user.isAdministrator
            ? "管理员免积分"
            : `${account.wallet.availablePoints} 积分`}
        </div>
      </header>
      <form className="creator-layout video-creator-layout" onSubmit={submit}>
        <section className="video-stage">
          <div className="video-stage-heading">
            <div>
              <span>VIDEO STUDIO</span>
              <h1>{config.title}</h1>
              <p>{config.description} 可上传新素材，或直接引用内容资产。</p>
            </div>
          ...
          <div className="ad-video-inputs">
            <div className="ad-input-heading">
              <strong>
                {template === "ad" ? "产品图片" : "产品与参考素材"} <em>*</em>
              </strong>
        ...
        <aside className="creator-panel">
          <div className="panel-title">
            ...
            <div>
              ...
              <p>
                ...
                {account.user.isAdministrator
                  ? `管理员免积分（报价 ${template === "mix" ? mixPoints : 40} 积分）`
                  : template === "mix"
                    ? `消耗 ${mixPoints} 积分`
                    : "消耗 40 积分"}
              </p>
          ...
          <label className="field-label">视频画面比例</label>
          ...
          <div className="ratio-control three-options">
            {(template === "mix" ? [15, 30, 45, 60] : durations).map((item) => (
              <button
                type="button"
                key={item}
          ...
          <label className="field-label">视频分辨率</label>
          <div className="ratio-control three-options">
          ...
          {template === "mix" && (
            <label className="recreate-consent">
              ...
              />
              我确认拥有全部视频素材的下载、剪辑和商业使用授权，并同意平台记录本次确认。
            </label>
          )}
          {error && (
          ...
          <button
            ...
            disabled={
              ...
              busy ||
              (!account.user.isAdministrator &&
                account.wallet.availablePoints <
                  (template === "mix" ? mixPoints : 40))
            }
          >
            <Upload size={18} />
            {busy
      ...
      {librarySlot && (
        <div
          ...
          aria-label="选择素材"
        >
          <section className="asset-picker-modal">
            <header>
              <div>
                <span>内容资产</span>
                <h2>
              ...
              <div className="asset-picker-grid">
                {compatibleAssets.map((asset) => (
                  <button
                    type="button"
                    key={asset.id}
                    onClick={() => selectAsset(librarySlot, asset)}
                  >
                    <span>
                      <Film size={24} />
                      视频素材
