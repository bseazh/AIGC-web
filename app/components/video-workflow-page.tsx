...
type UploadSlot = "image1" | "image2" | "video" | "audio";
...
const allDefinitions: Definition[] = [
  { key: "image1", label: "首帧商品图", hint: "必传 · JPG / PNG / WebP", accepts: "image/jpeg,image/png,image/webp", icon: Plus },
  { key: "image2", label: "尾帧或成品图", hint: "可选 · 用于收束画面", accepts: "image/jpeg,image/png,image/webp", icon: Plus },
  { key: "video", label: "参考视频", hint: "可选 · MP4，最多 100MB", accepts: "video/mp4", icon: Film },
  { key: "audio", label: "参考音频", hint: "可选 · MP3 / WAV，最多 30MB", accepts: "audio/mpeg,audio/mp3,audio/wav", icon: Music2 },
];

const templateConfig = {
  ad: { title: "产品广告大片", subtitle: "商品图驱动的 15 秒广告短片", submitUrl: "/api/tasks/product-ad-video/", description: "自动组织开场、卖点细节与收束尾帧", keys: ["image1", "image2", "audio"] as UploadSlot[], scenes: ["商品特写", "使用瞬间", "开箱展示", "氛围陈列"], styles: ["质感广告", "轻快节奏", "清新生活", "高端大片"] },
  recreate: { title: "复刻带货视频", subtitle: "参考节奏，生成原创商品内容", submitUrl: "/api/tasks/recreate-video/", description: "仅提取运镜、景别和转场结构，不复制原内容", keys: ["image1", "video", "audio"] as UploadSlot[], scenes: ["镜头节奏复刻", "商品展示复刻", "种草讲解复刻", "场景切换复刻"], styles: ["自然带货", "轻快节奏", "质感种草", "促销转化"] },
  seedance: { title: "Seedance2 视频", subtitle: "图、视频、音频自由组合的高级创作", submitUrl: "/api/tasks/video/", description: "完整参考素材与自由脚本控制", keys: ["image1", "image2", "video", "audio"] as UploadSlot[], scenes: ["商品特写", "第一人称", "生活方式", "自由创作"], styles: ["轻快节奏", "质感广告", "真实记录", "电影感"] },
} as const;
...
export function VideoWorkflowPage({ template = "seedance" }: Props) {
  ...
  const [scene, setScene] = useState<string>(config.scenes[0]);
  const [style, setStyle] = useState<string>(config.styles[0]);
  ...
  const upload = async (item: Uploaded) => {
    ...
    const stored = await fetch(presign.uploadUrl, { method: "PUT", headers: { "Content-Type": file.type }, body: file }); if (!stored.ok) throw new Error("素材上传失败");
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!uploads.image1 || phase === "uploading" || phase === "generating") return;
    ...
    try {
      ...
      const response = await fetch(config.submitUrl, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ assetIds, prompt, aspectRatio: ratio, scene, style }) });
  ...
  return <main className="creator-shell video-creator-shell"><header className="creator-header"><button className="icon-button" aria-label="返回视频中心" onClick={() => router.push("/create/product-video")}><ArrowLeft size={19} /></button><div><strong>{config.title}</strong><span>{config.subtitle}</span></div><div className="creator-points"><Sparkles size={15} />{account.wallet.availablePoints} 积分</div></header>
    <form className="creator-layout video-creator-layout" onSubmit={submit}><section className="video-stage"><div className="video-stage-heading"><div><span>VIDEO STUDIO</span><h1>{config.title}</h1><p>{config.description}。可上传新素材，或直接引用内容资产。</p></div><div className="video-badge"><Film size={16} />15 秒 · Seedance 2.0</div></div><div className="video-upload-grid">{definitions.map((definition) => { const item = uploads[definition.key]; const Icon = definition.icon; return <label className={`video-upload-card ${item ? "has-file" : ""}`} key={definition.key}>{item && definition.key.startsWith("image") ? <img src={item.preview} alt={definition.label} /> : item ? <div className="file-preview"><Icon size={28} /><strong>{item.name}</strong><small>{Math.ceil(item.byteSize / 1024 / 1024)} MB</small></div> : <><span><Icon size={22} /></span><strong>{definition.label}{definition.required ? " *" : ""}</strong><small>{definition.hint}</small></>}<button type="button" className="asset-picker-button" onClick={(event) => { event.preventDefault(); openLibrary(definition.key); }}><FolderOpen size={14} />素材库</button>{item && <button type="button" aria-label={`移除${definition.label}`} onClick={(event) => { event.preventDefault(); if (item.file) URL.revokeObjectURL(item.preview); setUploads((current) => { const next = { ...current }; delete next[definition.key]; return next; }); }}><X size={15} /></button>}<input type="file" accept={definition.accepts} onChange={(event) => choose(definition.key, event.target.files?.[0])} /></label>; })}</div>{busy && <div className="generation-overlay"><LoaderCircle size={30} /><strong>{phase === "uploading" ? "正在上传参考素材" : "Seedance 正在合成视频"}</strong><span>通常需要数分钟，完成后将自动保存至内容资产</span></div>}{phase === "succeeded" && result?.outputs[0] && <div className="video-result"><video src={result.outputs[0].url} controls playsInline /><a href={result.outputs[0].url} target="_blank" rel="noreferrer">打开视频</a></div>}</section>
      <aside className="creator-panel"><div className="panel-title"><span><Film size={18} /></span><div><h1>视频设置</h1><p>固定 15 秒，消耗 40 积分</p></div></div><label className="field-label">画幅比例</label><div className="ratio-control"><button type="button" className={ratio === "16:9" ? "active" : ""} onClick={() => setRatio("16:9")}>16:9 横版</button><button type="button" className={ratio === "9:16" ? "active" : ""} onClick={() => setRatio("9:16")}>9:16 竖版</button></div><label className="field-label">镜头方向<select value={scene} onChange={(event) => setScene(event.target.value)}>{config.scenes.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field-label">节奏风格<select value={style} onChange={(event) => setStyle(event.target.value)}>{config.styles.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field-label">视频脚本<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} maxLength={1200} placeholder="描述镜头、商品卖点、动作和希望保留的画面重点" /><small>{prompt.length}/1200</small></label>{error && <p className="creator-error" role="alert">{error}</p>}{phase === "succeeded" && <p className="creator-success"><Check size={16} />视频已保存到内容资产</p>}<button className="generate-button" type="submit" disabled={missingRequired || busy || account.wallet.availablePoints < 40}><Upload size={18} />{busy ? "任务处理中" : missingRequired ? "请补充必传素材" : account.wallet.availablePoints < 40 ? "积分不足" : `生成${config.title}`}</button></aside></form>{librarySlot && <div className="asset-picker-backdrop" role="dialog" aria-modal="true" aria-label="选择素材"><section className="asset-picker-modal"><header><div><span>内容资产</span><h2>选择{definitions.find((definition) => definition.key === librarySlot)?.label}</h2></div><button type="button" className="icon-button" onClick={() => setLibrarySlot(null)}><X size={18} /></button></header>{assetsLoading ? <div className="asset-picker-empty"><LoaderCircle size={22} />正在加载素材</div> : compatibleAssets.length === 0 ? <div className="asset-picker-empty"><FolderOpen size={25} /><strong>暂无可用素材</strong><p>先在图片创作或上传后完成校验，素材会自动显示在这里。</p></div> : <div className="asset-picker-grid">{compatibleAssets.map((asset) => <button type="button" key={asset.id} onClick={() => selectAsset(librarySlot, asset)}>{asset.mimeType.startsWith("image/") ? <img src={asset.url} alt="" /> : <span><Film size={24} />{asset.mimeType.startsWith("audio/") ? "音频素材" : "视频素材"}</span>}<strong>{asset.originalName}</strong><small>{asset.kind === "OUTPUT" ? "生成结果" : "上传素材"}</small></button>)}</div>}</section></div>}</main>;
}
