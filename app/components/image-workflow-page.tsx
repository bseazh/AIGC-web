...
export function ImageWorkflowPage({ title, description, submitUrl, scenes, styles, sourceTitle, sourceHint, submitLabel, pointsPerTask = 10, outputCount = 4, showAspectRatio = true, sceneLabel = "使用场景", styleLabel = "视觉风格" }: Props) {
  ...
  return <main className="creator-shell">
    ...
    <form className="creator-layout" onSubmit={submit}>
      <section className="upload-stage">
        ...
        {busy && <div className="generation-overlay"><LoaderCircle size={30} /><strong>{phase === "uploading" ? "正在上传并校验素材" : `正在生成 ${outputCount} 张${title}`}</strong><span>任务由服务器异步处理</span></div>}
      ...
      <aside className="creator-panel"><div className="panel-title"><span><Sparkles size={18} /></span><div><h1>生成设置</h1><p>{description}</p></div></div>
        ...
        {error && <p className="creator-error" role="alert">{error}</p>}{phase === "succeeded" && <p className="creator-success"><Check size={16} />{outputCount} 张结果已保存到内容资产</p>}
        <button className="generate-button" type="submit" disabled={!file || busy || account.wallet.availablePoints < pointsPerTask}><Upload size={18} />{busy ? "任务处理中" : account.wallet.availablePoints < pointsPerTask ? "积分不足" : submitLabel}</button>
      </aside>
    </form>
  </main>;
}
