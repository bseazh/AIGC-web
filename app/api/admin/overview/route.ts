...
export async function GET(request: NextRequest) {
  ...
  const like = `%${query}%`;
  const [users, tasks, assets, ledger, storage, operations] = await Promise.all([
    ...
    db.query<{ id: string; display_name: string; identifier: string; quota_bytes: string; used_bytes: string }>(`SELECT u.id, u.display_name, COALESCE(u.email, u.phone, '-') AS identifier, COALESCE(q.quota_bytes, 1073741824)::text AS quota_bytes, COALESCE(SUM(a.byte_size) FILTER (WHERE a.audit_status IN ('UPLOADING', 'READY')), 0)::text AS used_bytes FROM users u LEFT JOIN user_storage_quotas q ON q.user_id = u.id LEFT JOIN assets a ON a.owner_id = u.id WHERE $1 = '' OR u.display_name ILIKE $2 OR u.email ILIKE $2 OR u.phone ILIKE $2 GROUP BY u.id, q.quota_bytes ORDER BY used_bytes::bigint DESC LIMIT 50`, [query, like]),
  ...
  return NextResponse.json({ users: users.rows.map((x) => ({ ...x, identifier: x.email || x.phone || "-" })), tasks: tasks.rows.map((x) => ({ ...x, workflowName: workflowName(x.workflow_key), statusLabel: taskStatusLabel(x.status) })), assets: assets.rows, ledger: ledger.rows, storage: storage.rows, operations: operations.rows });
}
