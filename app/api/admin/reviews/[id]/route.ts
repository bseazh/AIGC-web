...
type ReviewRow = {
  ...
  workflow_key: string | null; task_status: string | null; points: number | null; input_json: Record<string, unknown> | null; email: string | null;
};
    ...
    if (action === "ESCALATE") {
      ...
      if (review.task_id && review.task_status === "PENDING_REVIEW") {
        ...
        if (Number(remaining.rows[0]?.count || 0) === 0) {
          ...
          if (task.rows[0]?.status === "PENDING_REVIEW") {
            const adminExempt = isAdminExemptTask(review.input_json);
      ...
      if (review.task_id && review.task_status === "PENDING_REVIEW") {
        ...
        if (task.rows[0]?.status === "PENDING_REVIEW") {
          ...
          await enqueueTaskNotification(client, { id: review.task_id, userId: review.owner_id, email: review.email, workflowKey: review.workflow_key || "generation", points: review.points || 0, adminExempt }, "REJECTED");
        }
      } else if (!review.task_id) {
        const waiting = await client.query<{ id: string; workflow_key: string; points: number; input_json: Record<string, unknown> }>(
          `SELECT id, workflow_key, points, input_json FROM generation_tasks
        ...
        if (waiting.rowCount) {
          ...
          const total = waiting.rows.reduce((sum, task) => sum + (isAdminExemptTask(task.input_json) ? 0 : task.points), 0);
          if (total > 0) await client.query("UPDATE wallets SET available_points = available_points + $2, frozen_points = frozen_points - $2, version = version + 1, updated_at = NOW() WHERE user_id = $1", [review.owner_id, total]);
          for (const waitingTask of waiting.rows) {
            const adminExempt = isAdminExemptTask(waitingTask.input_json);
            if (!adminExempt) balance += waitingTask.points;
            ...
            if (!adminExempt) await client.query(
              `INSERT INTO wallet_ledger (user_id, type, amount, balance_after, business_type, business_id, idempotency_key)
               VALUES ($1, 'REFUND', $2, $3, 'GENERATION_TASK', $4, $5) ON CONFLICT (idempotency_key) DO NOTHING`,
              [review.owner_id, waitingTask.points, balance, waitingTask.id, `refund:${waitingTask.id}`],
            );
            await enqueueTaskNotification(client, { id: waitingTask.id, userId: review.owner_id, email: review.email, workflowKey: waitingTask.workflow_key, points: waitingTask.points, adminExempt }, "REJECTED");
          }
        }
      }
    }
...
async function refundQueueFailure(taskId: string) {
  ...
  try {
    await client.query("BEGIN");
    const found = await client.query<{ user_id: string; workflow_key: string; points: number; status: string; input_json: Record<string, unknown>; email: string | null }>("SELECT t.user_id, t.workflow_key, t.points, t.status, t.input_json, u.email FROM generation_tasks t JOIN users u ON u.id = t.user_id WHERE t.id = $1 FOR UPDATE OF t", [taskId]);
    const task = found.rows[0];
    if (task?.status === "QUEUED") {
      const adminExempt = isAdminExemptTask(task.input_json);
      ...
      await enqueueTaskNotification(client, { id: taskId, userId: task.user_id, email: task.email, workflowKey: task.workflow_key, points: task.points, adminExempt }, "FAILED");
    }
    await client.query("COMMIT");
