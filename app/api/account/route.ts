...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    await client.query("UPDATE users SET display_name = $2, password_hash = COALESCE($3, password_hash), token_version = $4, avatar_style = $5, avatar_asset_id = COALESCE($6, avatar_asset_id), updated_at = NOW() WHERE id = $1", [user.id, displayName, newPassword ? await hashPassword(newPassword) : null, tokenVersion, avatarStyle, avatarAssetId]);
    await client.query("COMMIT");
    const response = NextResponse.json({ displayName, passwordChanged: Boolean(newPassword), avatarStyle, avatarAssetId });
