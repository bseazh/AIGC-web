    ...
    if (action === "recommend") {
      ...
      const result = await callAssistantLLM({
        ...
        userPrompt: [
          ...
          "audiences 由你主动推断 1-2 个最合适的人群，仅供内部生成使用，不要求用户选择。",
          "scenes、styles、sellingPoints 各返回 3 个简短、差异明显、可点击的中文选项，优先给可直接成片的具体方案，不要把细节拆成连续问答。",
          ...
          "reply 用不超过 120 字说明你如何理解商品，并邀请用户从三组方向中快速选择。",
          "question 必须返回空字符串，quickReplies 必须返回空数组。除非商品身份无法识别，否则不要向用户追问。受众、镜头、张数、构图和系列一致性由你结合当前模块主动推断。",
          `目标图片类型：${goal}`,
