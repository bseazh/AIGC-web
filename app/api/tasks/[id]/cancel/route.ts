...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!adminExempt) await client.query("UPDATE wallets SET available_points = available_points + $2, frozen_points = frozen_points - $2, version = version + 1, updated_at = NOW() WHERE user_id = $1", [user.id, points]);
  ...
  await audit(user.id, "TASK_CANCELED", request, { type: "generation_task", id }, { previousStatus, refundedPoints: adminExempt ? 0 : points, adminExempt });
  return NextResponse.json({ taskId: id, status: "CANCELED", refundedPoints: adminExempt ? 0 : points, adminExempt });
}
