...
export default function AssetsPage() {
  ...
  const filteredAssets = assets.filter((asset) => matchesMediaType(asset, mediaType));
  ...
  const sections = kind === "INPUT"
    ...
    : kind === "OUTPUT"
      ? [{ key: "OUTPUT", kicker: "GENERATED", title: "生成结果", assets: generatedAssets }]
      : [
          { key: "INPUT", kicker: "UPLOADED", title: "上传素材", assets: uploadedAssets },
          { key: "OUTPUT", kicker: "GENERATED", title: "生成结果", assets: generatedAssets },
        ];
  ...
  return <AppShell active="assets" account={account}>
    <div className="app-page-content">
      ...
      <section className="asset-toolbar"><div className="filter-tabs">{kinds.map((item) => <button key={item.key} className={kind === item.key ? "active" : ""} onClick={() => { setKind(item.key); setPreviewAsset(null); }}>{item.label}</button>)}</div><div className="filter-tabs asset-type-tabs">{mediaFilters.map((item) => <button key={item.key} className={mediaType === item.key ? "active" : ""} onClick={() => { setMediaType(item.key); setPreviewAsset(null); }}>{item.label}</button>)}</div><label className="asset-search"><Search size={16} /><input value={query} onChange={(event) => { setQuery(event.target.value); setPreviewAsset(null); }} placeholder="搜索资产名称" /></label><span>共 {filteredAssets.length} 项</span></section>
      {loading ? <div className="records-loading"><LoaderCircle size={22} />正在载入资产</div> : filteredAssets.length === 0 ? <div className="page-empty standalone"><span><Boxes size={27} /></span><strong>暂无匹配资产</strong><p>上传商品素材或完成生成后，内容会自动保存在这里。</p><Link href={projectGateHref("product-hero-image")}>开始创作</Link></div> : <div className="asset-sections">{sections.map((section) => <section className="asset-section" key={section.key}><div className="asset-section-title"><div><span>{section.kicker}</span><h2>{section.title}</h2></div><p>{section.assets.length} 项</p></div>{section.assets.length ? <div className="asset-grid">{section.assets.map(assetCard)}</div> : <div className="asset-section-empty">暂无{section.title}</div>}</section>)}</div>}
    </div>
