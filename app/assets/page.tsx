...
export default function AssetsPage() {
  ...
  return <AppShell active="assets" account={account}>
    <div className="app-page-content">
      ...
      {loading ? <div className="records-loading"><LoaderCircle size={22} />正在载入资产</div> : filteredAssets.length === 0 ? <div className="page-empty standalone"><span><Boxes size={27} /></span><strong>暂无匹配资产</strong><p>上传商品素材或完成生成后，内容会自动保存在这里。</p><Link href="/create/product-hero">开始创作</Link></div> : <div className="asset-sections">{sections.map((section) => <section className="asset-section" key={section.key}><div className="asset-section-title"><div><span>{section.kicker}</span><h2>{section.title}</h2></div><p>{section.assets.length} 项</p></div>{section.assets.length ? <div className="asset-grid">{section.assets.map(assetCard)}</div> : <div className="asset-section-empty">暂无{section.title}</div>}</section>)}</div>}
    </div>
