...
function ModelWearWorkspace() {
  ...
  const resetForm = () => { if (model) URL.revokeObjectURL(model.preview); products.forEach((item) => URL.revokeObjectURL(item.preview)); setModel(null); setProducts([]); setRatio("1:1"); setScene(scenes[0]); setStyle(styles[0]); setPrompt(""); setOutputCount(1); setAppliedCaseId(""); setError(""); setTask(null); setPhase("idle"); };
