...
import { ArrowLeft, Check, Download, ImagePlus, Plus, Sparkles, Upload, Wand2, X } from "lucide-react";
...
function ModelWearWorkspace() {
  ...
  return <main className="creator-shell"><header className="creator-header"><button className="icon-button" aria-label="返回工作台" onClick={() => router.push("/workspace")}><ArrowLeft size={19} /></button><div><strong>模特穿搭</strong><span>芭乐AIGC</span></div><div className="creator-points"><Sparkles size={15} />{account.user.isAdministrator ? "管理员免积分" : `${account.wallet.availablePoints} 积分`}</div></header>
    <form className="creator-layout inspiration-workflow-layout wear-layout" onSubmit={submit}><section className="wear-stage"><div className="wear-inputs"><section className="wear-source"><div className="wear-source-title"><span>1</span><div><strong>模特图</strong><small>建议上传清晰的正面或全身照片</small></div></div>{model ? <div className="wear-preview single"><img src={model.preview} alt="模特素材" /><button type="button" aria-label="移除模特图" onClick={() => { URL.revokeObjectURL(model.preview); setModel(null); }}><X size={16} /></button></div> : <label className="wear-drop"><ImagePlus size={24} /><span>上传模特图</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setModelFile(event.target.files?.[0])} /></label>}</section>
      ...
      {busy && <GenerationProgress phase={phase} taskStatus={task?.status} title="模特穿搭图" outputCount={4} />}{phase === "succeeded" && task && <div className="result-grid">{task.outputs.map((output, index) => <article key={output.assetId}><img src={output.url} alt={`模特穿搭结果 ${index + 1}`} /><a href={output.url} download target="_blank" rel="noreferrer"><Download size={16} />下载</a></article>)}</div>}</section>
      <aside className="creator-panel"><div className="panel-title"><span><Sparkles size={18} /></span><div><h1>生成设置</h1><p>{account.user.isAdministrator ? "管理员免积分 · 报价 10 积分计入成本审计" : "一次生成 4 张，消耗 10 积分"}</p></div></div><label className="field-label">画幅比例</label><div className="ratio-control">{ratios.map((item) => <button type="button" key={item} className={ratio === item ? "active" : ""} onClick={() => setRatio(item)}>{item}</button>)}</div><label className="field-label">使用场景<select value={scene} onChange={(event) => setScene(event.target.value)}>{scenes.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field-label">视觉风格<select value={style} onChange={(event) => setStyle(event.target.value)}>{styles.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field-label">补充要求<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} maxLength={1200} placeholder="例如：保持模特发型，展示全身穿搭效果" /><small>{prompt.length}/1200</small></label>{error && <p className="creator-error" role="alert">{error}</p>}{phase === "succeeded" && <p className="creator-success"><Check size={16} />4 张结果已保存到内容资产</p>}<button className="generate-button" type="submit" disabled={!model || !products.length || busy || (!account.user.isAdministrator && account.wallet.availablePoints < 10)}><Upload size={18} />{busy ? "任务处理中" : !account.user.isAdministrator && account.wallet.availablePoints < 10 ? "积分不足" : "生成模特穿搭图"}</button></aside>
      <aside className="case-reference-panel">
        <header><div><h2>案例参考</h2><p>选择案例可一键回填穿搭方向</p></div></header>
