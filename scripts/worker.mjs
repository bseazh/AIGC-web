...
async function generateOne(inputUrls, input, index, workflowKey, generationTaskId) {
  ...
  const recreateReferencePrompt = workflowKey === "recreate-reference-image" && input.scene === "人物多视图"
    ? `创建一张 16:9 人物/模特角色身份板，不是单张商品图，也不是空白网格。第一张输入图如果出现真人、模特、人体轮廓、头发、脸、手臂、腿或穿在人身上的服装，必须把“完整人物”作为唯一主主体来提取和重建；服装只是穿在人物身上的附着物，不得把服装单独当商品。画面必须在同一张图里包含多个清晰分离的角色研究：一个大型完整站姿英雄全身视角、一个背面全身视角、一个侧面全身视角、一个 3/4 角度全身视角、一个上半身脸型与发型轮廓研究、一个服装/姿态细节研究。每个视角都必须是同一位虚拟模特，保持相同脸型轮廓、发型轮廓、身体比例、服装轮廓和姿态气质。背景纯白或柔和米白，大量留白，角色图像不要重叠，不要裁剪头部，不要隐藏肢体。禁止输出只有裙子、空心衣服、衣架、平铺服装、无头模特、无脸空白块、空白分格或商品白底图。不要在生图阶段遮挡脸部；生成原创虚拟脸型轮廓，不复制真实五官，后续系统只会二次遮挡脸部五官区域。画幅比例${input.aspectRatio}。`
