...
const generation = new Queue("generation", { connection }); const moderation = contentReviewEnabled ? new Queue("moderation", { connection }) : null;
...
try {
  const [taskStats, reviewStats, refundStats, complaintStats, realUsers, rollout, operationChecks, generationWaiting, generationActive, moderationWaiting, httpTotal, http5xx] = await Promise.all([
    ...
    generation.getWaitingCount(), generation.getActiveCount(), moderation ? moderation.getWaitingCount() : 0,
  ...
  if (contentReviewEnabled && Number(reviews.overdue) > 0) blockers.push("REVIEW_SLA");
...
} finally { await generation.close(); if (moderation) await moderation.close(); await pool.end(); }
