...
export async function createImageTask(request: NextRequest, workflow: ImageWorkflow, selectAssets: AssetSelector = (body) => [typeof body.assetId === "string" ? body.assetId : ""], validateAssets?: AssetValidator, validateRequest?: RequestValidator, inputExtras?: InputExtras) {
  ...
  try {
    ...
    const caseRecreationPrompt = typeof body.caseRecreationPrompt === "string" ? body.caseRecreationPrompt.trim().slice(0, 2400) : "";
    const input = { assetId: assets[0]?.id || null, storageKey: assets[0]?.storage_key || null, assetIds: assets.map((asset) => asset.id), storageKeys: assets.map((asset) => asset.storage_key), assetMimeTypes: assets.map((asset) => asset.mime_type), prompt, aspectRatio, aspectRatioMode: imageAspectRatio.mode, requestedAspectRatio: imageAspectRatio.requested, customAspectRatioWidth: imageAspectRatio.width, customAspectRatioHeight: imageAspectRatio.height, duration, resolution, ...(acceptsStructuredDirection ? { scene, style } : {}), internalPrompt: workflow.internalPrompt || "", ...(caseRecreationPrompt ? { caseRecreationPrompt } : {}), outputs: requestedOutputCount(body, workflow), ...(detailCards.length ? { detailCards } : {}), ...(singleCard ? { singleCardIndex: Number(body.singleCardIndex) } : {}), ...(seriesPlan.length ? { seriesPlan } : {}), ...(visualBible ? { visualBible } : {}), ...(adminExempt ? { billingMode: ADMIN_EXEMPT_BILLING_MODE, quotedPoints: points } : {}), ...(promptConfig ? { promptConfig } : {}), ...(inputExtras?.(body) || {}) };
    await client.query(
