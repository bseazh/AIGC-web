...
export const recreateReferenceWorkflow = {
  ...
  name: "复刻素材参考图",
  enabled: Boolean((process.env.GOOGLE_AI_API_KEY || process.env.GEMINI_API_KEY || process.env.NANO_BANANA_API_KEY) && process.env.COS_SECRET_ID),
  disabledReason: (process.env.GOOGLE_AI_API_KEY || process.env.GEMINI_API_KEY || process.env.NANO_BANANA_API_KEY) && process.env.COS_SECRET_ID ? null : "PROVIDER_NOT_CONFIGURED",
